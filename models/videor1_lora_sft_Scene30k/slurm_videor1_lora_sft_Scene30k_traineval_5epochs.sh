...
# ---------------------------------------------------------------------
# ------------ videor1_lora_sft_Scene30k_traineval_5epochs ----------
# ---------------------------------------------------------------------
#
# This script will train (SFT, LoRA) a VideoR1 model on the Scene30k dataset for 5 epochs.
#

# ----- HEADER: ENV VARIABLES -----

EXPERIMENT_NAME="videor1_lora_sft_Scene30k_traineval_5epochs"
...
OUTPUT_DIR="${PROJECT_DIR}/saves/videor1/lora/sft/Scene30k_traineval_5epochs"
...
if [[ "$CLUSTER" == "RORQUAL" ]]; then
    ...
    if [[ "$RUNNING_MODE" == "APPTAINER" ]]; then
        ...
        #     --dataloader_pin_memory false \
        #     --low_cpu_mem_usage \
        #     --output_dir /project/aip-wangcs/indrisch/LLaMA-Factory/saves/videor1/lora/sft/Scene30k_traineval \
        #     --logging_steps 10 \
        #     --save_steps 200 \
        #     --plot_loss \
        #     --overwrite_output_dir \
