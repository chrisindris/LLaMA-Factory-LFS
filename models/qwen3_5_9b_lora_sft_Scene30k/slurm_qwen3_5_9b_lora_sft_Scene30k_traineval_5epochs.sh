...
#SBATCH --error=err/%N-qwen3_5_9b_lora_sft_Scene30k_traineval_5epochs-%j.err
...
# ---------------------------------------------------------------------
# ------------ qwen3_5_9b_lora_sft_Scene30k_traineval_5epochs ----------
# ---------------------------------------------------------------------
#
...
EXPERIMENT_NAME="qwen3_5_9b_lora_sft_Scene30k_traineval_5epochs"
...
if [[ "$PS1" == *"rorqual"* ]] || [[ "$HOSTNAME" == *"rorqual"* ]] || [[ "$PS1" == *"rg"* ]] || [[ "$HOSTNAME" == *"rg"* ]]; then
    ...
    CLUSTER="TRILLIUM"
    RUNNING_MODE="VENV" # running mode for TRILLIUM
...
if [[ "$CLUSTER" == "RORQUAL" ]]; then
    ...
    if [[ "$RUNNING_MODE" == "APPTAINER" ]]; then
        ...
        # FOR DEBUGGING 544942.out:
        export DISABLE_CUDNN_SDP=1
    ...
    if [[ "$RUNNING_MODE" == "APPTAINER" ]]; then
        ...
        lmod_preflight
        # -- using cu12.9 due to the error in *3662965.out can be done; however, let's use cu12.6 since it works in the 2node version.
        # module load StdEnv/2023  gcc/12.3  openmpi/4.1.5
        # module load python/3.12 cuda/12.6 opencv/4.12.0
        # module load arrow
