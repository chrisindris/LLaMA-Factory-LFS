...
#SBATCH --array=0-4
#SBATCH --output=out/%N-qwen2_5vl_eval_basemodel_on_dataset-%j.out

# we are going to evaluate the base Qwen2.5-VL-7B-Instruct model on the datasets [SQA3D, X62, R1, C1, X1]
# SLURM_ARRAY_TASK_ID=4 # temporary, for debugging
...
MODEL_INDEX=$((SLURM_ARRAY_TASK_ID / 5)) # will always be 0 if 0 <= SLURM_ARRAY_TASK_ID <= 4
DATASET_INDEX=$((SLURM_ARRAY_TASK_ID % 5))

MODEL_SHORTNAMES=("Qwen25VL7BInstruct")
...
MODEL_CHECKPOINTS=("null") # no adapter checkpoint needed for the base model
